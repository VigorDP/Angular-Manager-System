...
export class PropertyGroupComponent implements OnInit {
  ...
  licenseUrl = '';
  columns: STColumn[] = [
    { title: '物业集团公司名称', index: 'name' },
    { title: '下属物业公司', index: 'enterpriseName' },
  ...
  getData(pageIndex?: number) {
    ...
    this.api.getEnterpriseList(this.query).subscribe(res => {
      this.loading = false;
  ...
  addOrEditOrView(tpl: TemplateRef<{}>, type: 'add' | 'edit' | 'view' | 'view') {
    if (type === 'edit' || type === 'view') {
      this.api.getEnterpriseInfo({ id: this.selectedRow.id }).subscribe(res => {
        if (res.code === '0') {
    ...
    this.modalSrv.create({
      nzTitle: type === 'add' ? '新建物业集团公司' : type === 'edit' ? '编辑物业集团公司' : '查看物业集团公司',
      nzContent: tpl,
  ...
  checkValid() {
    const { name, delegate, contact, contactTel, taxCode, publicAccount, publicBank, bankAddress } = this.selectedRow;
    if (!name) {
      this.msg.info('请输入物业集团公司名称');
      return false;
    }
    if (!delegate) {
      this.msg.info('请输入法人');
  ...
  delete() {
    this.modalSrv.confirm({
      ...
      nzOkType: 'danger',
      nzOnOk: () => {
        this.api.deleteEnterprise([this.selectedRow.id]).subscribe(() => {
          this.getData();
          this.st.clearCheck();
        });
      },
